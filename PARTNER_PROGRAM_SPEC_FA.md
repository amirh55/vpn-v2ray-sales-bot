...
**فاز ۱ — پایه داده**
مدل‌های `Partner`، `PartnerPlanPrice`، `PartnerRequest` + فیلدهای `Order`،
`Payment`، `CardPaymentRequest`، `SiteSetting`، `Plan` + مهاجرت `revenue_at`
با backfill. ادمین این‌ها. هیچ تغییری در رفتار ربات. **قابل دیپلوی به‌تنهایی.**
